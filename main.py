...
while running:
  ...
  if column != previous_column and board.complete == Board.NOT_COMPLETE:
    if column != -1: view.highlight_selected_chip(column, board.find_lowest_empty_row(column), True)
    if previous_column != -1: view.highlight_selected_chip(previous_column, board.find_lowest_empty_row(previous_column), False)
